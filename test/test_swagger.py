...
class TestSwagger(BaseTest):
    ...
    def test_multiple_query_strings(self):
        ...
        assert float(val_1), "last_qery_url: {}, value1: {}".format(
            last_call["req_url"], val_1
        )
        assert not isinstance(val_2, int), "last_qery_url: {}, value2: {}".format(
            last_call["req_url"], val_2
        )
        self.repot_basic_check()
    ...
    def test_multiple_path_params(self):
        ...
        _, test_url_reported, param1, param2 = last_call["req_path"].split(
            "/", maxsplit=3
        )
        assert test_url[1:] == test_url_reported, last_call[
            "req_path"
        ]  # heading / shall be removed
        assert not isinstance(param1, int), "req_path: {}, param1: {}".format(
            last_call["req_url"], param1
        )
        assert not isinstance(param2, int), "req_path: {}, param2: {}".format(
            last_call["req_url"], param2
        )
        self.repot_basic_check()
    ...
    def test_v2_post_with_schema(self):
                ...
                "parameters": [
                    {
                        "in": "body",
                        "name": "body",
                        "schema": {"$ref": "#/definitions/schema_definition"},
                    }
                ]
            }
        }
        ...
        last_call = self.fuzz_swagger_and_get_last_call(
            api_path, api_def, schema_definitions=schema
        )
        # "req_form": {
        #     "param_int": "\u0000",
        #     "param_str": "65Y"

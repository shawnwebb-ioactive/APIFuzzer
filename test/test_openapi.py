...
class TestOpenAPI(BaseTest):
        ...
        api_def = {
            "post": {
                "requestBody": {
                    "content": {
                        "application/json": {
                            "schema": {"$ref": "#/definitions/ItemDraft"}
                        }
                    }
                }
            }
        }
        last_call = self.fuzz_openapi_and_get_last_call(
            api_path, api_def, schema_definitions=self.schema
        )
    ...
    def test_v3_post_with_schema_multiparm_formdata_content_type(self):
        ...
        assert last_call["req_headers"]["Content-Type"].startswith(
            "multipart/form-data"
        )
        self.repot_basic_check()
